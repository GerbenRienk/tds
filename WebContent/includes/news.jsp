
	<div class="secondary">
		<!-- 
		<div align='right'><a href='/tds/search.jsp'>search the site</a></div>
		 -->

<img src='/tds/img/TDS_logo.png'  width='100%'><br />&nbsp;
<div class="modnews">
			<h3>
			<a href='/tds/newsall.jsp' title='click for more details'>
			News
			</a>
			</h3>
			<ul class="press">

<li>
	<blockquote>
		<p>libreclinica v.1 released</p>
		<p class="author"><a href='/tds/newsall.jsp#N057' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		27-2-2020</p>
	</blockquote>
</li>
<li>
	<blockquote>
		<p>defaults in a grid</p>
		<p class="author"><a href='/tds/newsall.jsp#N056' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		9-2-2018</p>
	</blockquote>
</li>
<li>
	<blockquote>
		<p>an automatic row number</p>
		<p class="author"><a href='/tds/newsall.jsp#N055' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		9-1-2017</p>
	</blockquote>
</li>
<li>
	<blockquote>
		<p>tomcat6 <span class='amp'>&amp;</span> 7</p>
		<p class="author"><a href='/tds/newsall.jsp#N052' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		18-8-2015</p>
	</blockquote>
</li>
<li>
	<blockquote>
		<p>workshop java-script</p>
		<p class="author"><a href='/tds/newsall.jsp#N050' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		12-7-2015</p>
	</blockquote>
</li>
<li>
	<blockquote>
		<p>de-selecting radio's in a group</p>
		<p class="author"><a href='/tds/newsall.jsp#N049' title='read more about this'>&rarr; more</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		9-12-2014</p>
	</blockquote>
</li>
			</ul>
		</div>
	</div> <!-- /.secondary -->
